...
const outputPath = path.join(ROOT, 'src', 'scratch', 'ClashCollection-first10-migrated.json');
...
const firstTen = Array.isArray(sourceData) ? sourceData.slice(0, 10).map(toRecord) : [];

fs.writeFileSync(outputPath, JSON.stringify(firstTen, null, 2));
console.log(`Wrote ${firstTen.length} records to ${outputPath}`);
